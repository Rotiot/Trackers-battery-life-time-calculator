...
const BATTERY_TYPES = {
  primary: {
    ...
    practical_capacity_multiplier: 0.8, // =80%
  },
  rechargeable: {
    descr: "Rechargeable (Li-Po)",
    leakage_per_month: 0.05, // =5%
    practical_capacity_multiplier: 0.9, // =90%
  },
};
...
const PRODUCTS = {
  industrial: {
    ...
    battery: {
      ...
      practical_capacity:
        19000 * BATTERY_TYPES.primary.practical_capacity_multiplier, // [mAh]
  ...
  compact: {
    ...
    battery: {
      ...
      practical_capacity:
        8000 * BATTERY_TYPES.primary.practical_capacity_multiplier, // [mAh]
      leakage_per_month: BATTERY_TYPES.primary.leakage_per_month, // ratio
    },
  },
  micro: {
    ...
    battery: {
      ...
      practical_capacity:
        450 * BATTERY_TYPES.rechargeable.practical_capacity_multiplier, // [mAh]
  ...
  smart_badge: {
    ...
    battery: {
      ...
      practical_capacity:
        1300 * BATTERY_TYPES.rechargeable.practical_capacity_multiplier, // [mAh]
      leakage_per_month: BATTERY_TYPES.rechargeable.leakage_per_month, // ratio
    },
  },
};
...
const QUIESENT_CURRENT = 0.001; // [mA] (=10 uA)
...
function calculate_lora_current(sf, tx_power, payl_len, nof_msg_per_day) {
  ...
  if (sf >= 11) {
    time_on_air =
      SYMBOL_TIME *
      (8 +
        Math.ceil(((payl_len + 12) * 8 - 4 * (sf - 7) + 16) / (4 * sf - 8)) *
          5);
  } else {
    time_on_air =
      SYMBOL_TIME *
      (8 +
        Math.ceil(((payl_len + 12) * 8 - 4 * (sf - 7) + 16) / (4 * sf - 0)) *
          5);
  }
  ...
  const tx_energy =
    (total_time_on_air * SUPPLY_VOLTAGE * TX_POWERS[tx_power].current) / 1000;
  const rx_energy = (2 * 8 * SYMBOL_TIME * SUPPLY_VOLTAGE * RX_CURRENT) / 1000; // 2 RX windows are open after every tx
  const mcu_energy =
    ((total_time_on_air + 2000) * SUPPLY_VOLTAGE * MCU_CURRENT) / 1000; // The MCU cannot sleep until the 2 RX windows are opened
  const total_energy_per_transmission = tx_energy + rx_energy + mcu_energy;

  const average_current =
    (total_energy_per_transmission / SUPPLY_VOLTAGE) *
    (nof_msg_per_day / (24 * 3600)); // [mA]
...
function calculate_gps_current(gps_ttff, gps_conv_time, nof_msg_per_day) {
  ...
  const average_current_with_cold_starts_only =
    (gps_usage_time_of_cold_start * GPS_CURRENT * nof_msg_per_day) /
    (24 * 3600);

  const average_current_with_hot_starts =
    // we assume that there is only one cold start per day
  ...
  const average_current = Math.min(
    average_current_with_cold_starts_only,
    average_current_with_hot_starts
  ); // [mA]
...
function calculate_agps_current(agps_on_time, nof_msg_per_day) {
  const average_current =
    (agps_on_time * GPS_CURRENT * nof_msg_per_day) / (24 * 3600); // [mA]
  return average_current; // in [mA]
}

function calculate_wifi_current(nof_msg_per_day) {
  const average_current =
    (WIFI_ON_TIME * WIFI_CURRENT * nof_msg_per_day) / (24 * 3600); // [mA]
  return average_current; // in [mA]
}

function calculate_ble_current(nof_msg_per_day) {
  const average_current =
    (BLE_ON_TIME * BLE_CURRENT * nof_msg_per_day) / (24 * 3600); // [mA]
  return average_current; // in [mA]
}

function calculate_scancollection_current(
  sf,
  nof_msg_per_day,
  tech,
  idtype,
...
function calculate_custom_ble_usage_current(operation, usage_time_per_day) {
  const average_current =
    (BLE_OPERATIONS[operation].current * usage_time_per_day) / 24; // [mA]
...
function calculate_battery_leakage_current(product) {
  ...
  const average_current =
    (battery_capacity / 2) * (battery_leakage_per_month / (30 * 24)); // [mA]
  return average_current; // [mA]
}

function calculate_battery_life_time(input) {
  const custom_msg_lora_current =
    input.nof_msg_repetition *
    calculate_lora_current(
      input.sf,
      input.tx_power,
      input.custom_msg.payl_len,
      input.custom_msg.nof_msg_per_day
    );
  const heartbeat_lora_current =
    ...
    calculate_lora_current(
      ...
      input.tx_power,
      HEARTBEAT_PAYL_LEN,
      input.heartbeat.nof_msg_per_day
    );
  ...
  const agps_lora_current =
    ...
    calculate_lora_current(
      ...
      input.tx_power,
      AGPS_MIN_PAYL_LEN +
        input.agps.nof_satellites * AGPS_ADDITIONAL_SAT_PAYL_LEN,
  ...
  if (
    input.scan_collection.tech == "BLE" &&
    input.scan_collection.idtype == "ID"
  ) {
  ...
  let nof_id_in_full_msg = Math.floor(
    (input.scan_collection.max_payl_len - SCANCOLL_MIN_PAYL_LEN) /
      scancoll_additional_payl_len
  );
  let nof_id_last_msg = input.scan_collection.nof_id % nof_id_in_full_msg;
  let nof_full_fragments = Math.floor(
    input.scan_collection.nof_id / nof_id_in_full_msg
  );
  ...
  const gps_geoloc_current = calculate_gps_current(
    input.gps.ttff,
    input.gps.conv_time,
    input.gps.nof_msg_per_day
  );
  const agps_geoloc_current = calculate_agps_current(
    input.agps.on_time,
    input.agps.nof_msg_per_day
  );
  const wifi_geoloc_current = calculate_wifi_current(
    input.wifi.nof_msg_per_day
  );
  ...
  const custom_ble_usage_current = calculate_custom_ble_usage_current(
    input.custom_ble.operation,
    input.custom_ble.usage_time_per_day
  );

  const scan_collection_current = calculate_scancollection_current(
    input.sf,
    input.scan_collection.nof_msg_per_day,
    input.scan_collection.tech,
    input.scan_collection.idtype,
    input.scan_collection.nof_id
  );

  const accelerometer_current = calculate_accelerometer_current(
    input.accelerometer_on
  );

  const average_battery_leakage = calculate_battery_leakage_current(
    input.product
  );

  const total_lora_current =
    custom_msg_lora_current +
    heartbeat_lora_current +
    gps_lora_current +
    agps_lora_current +
    wifi_lora_current +
    ble_lora_current +
    scan_collection_lora_current;
  ...
  const total_current =
    ...
    total_fix_current; // [mA]

  const battery_life_time =
    Math.round(
      (10 *
        (PRODUCTS[input.product].battery.practical_capacity / total_current)) /
        24
    ) / 10; // [day]

  const current_distribution = {
    custom_msg:
      Math.round((1000 * custom_msg_lora_current) / total_current) / 10,
    heartbeat: Math.round((1000 * heartbeat_lora_current) / total_current) / 10,
    gps:
      Math.round(
        (1000 * (gps_lora_current + gps_geoloc_current)) / total_current
      ) / 10,
    agps:
      Math.round(
        (1000 * (agps_lora_current + agps_geoloc_current)) / total_current
      ) / 10,
    wifi:
      Math.round(
        (1000 * (wifi_lora_current + wifi_geoloc_current)) / total_current
      ) / 10,
    ble:
      Math.round(
        (1000 * (ble_lora_current + ble_geoloc_current)) / total_current
      ) / 10,
    custom_ble:
      Math.round((1000 * custom_ble_usage_current) / total_current) / 10,
    scan_collection:
      Math.round(
        (1000 * (scan_collection_lora_current + scan_collection_current)) /
          total_current
      ) / 10,
    accelerometer:
      Math.round((1000 * accelerometer_current) / total_current) / 10,
    quiesent_and_battery_leakage:
      Math.round((1000 * total_fix_current) / total_current) / 10,
  };
