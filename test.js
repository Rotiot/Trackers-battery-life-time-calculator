const { calculate_battery_life_time } = require("./rotiot_battery.js");
...
if (result.current_distribution.custom_msg > 0) {
  result_text +=
    "  Custom msg: " + result.current_distribution.custom_msg + " %\n";
}
if (result.current_distribution.heartbeat > 0) {
  result_text +=
    "  Heartbeat: " + result.current_distribution.heartbeat + " %\n";
}
...
if (result.current_distribution.custom_ble > 0) {
  result_text +=
    "  Custom BLE: " + result.current_distribution.custom_ble + " %\n";
}
if (result.current_distribution.scan_collection > 0) {
  result_text +=
    "  Scan collecrion: " +
    result.current_distribution.scan_collection +
    " %\n";
}
if (result.current_distribution.accelerometer > 0) {
  result_text +=
    "  Accelerometer: " + result.current_distribution.accelerometer + " %\n";
}
if (result.current_distribution.quiesent_and_battery_leakage > 0) {
  result_text +=
    "  Quiesent and Battery leakage: " +
    result.current_distribution.quiesent_and_battery_leakage +
    " %\n";
}
